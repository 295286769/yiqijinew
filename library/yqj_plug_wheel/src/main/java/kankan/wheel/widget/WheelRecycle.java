...
public class WheelRecycle {
     ...
     * original layout.将layout中的item不在range中，则从layout中去除并缓存item，循环下一个item，判断是否为第一项。
     *
    ...
    public int recycleItems(LinearLayout layout, int firstItem, ItemsRange range) {
        ...
        for (int i = 0; i < layout.getChildCount(); ) {
            if (!range.contains(index)) {//如果第一个位置不在range中，则去除layout子项，设置下一项为首项，在range中，则循环的当前项的位置就是首项
