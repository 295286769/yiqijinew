...
public class ItemDragAndSwipeCallback extends ItemTouchHelper.Callback {
    ...
    public static final int defualt_drag_move_flags = ItemTouchHelper.UP | ItemTouchHelper.DOWN | ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT;

    IDragAndSwipeAdapter mdragAndSwipeAdapter;

    float mMoveThreshold = 0.1f;
    float mSwipeThreshold = 0.7f;

    int mDragMoveFlags = defualt_drag_move_flags;
    int mSwipeMoveFlags = ItemTouchHelper.END;


    public ItemDragAndSwipeCallback(IDragAndSwipeAdapter dragAndSwipeAdapter) {
        mdragAndSwipeAdapter = dragAndSwipeAdapter;
    }
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
        if (actionState == ItemTouchHelper.ACTION_STATE_DRAG
                && !mdragAndSwipeAdapter.isHeadOrFooterView(viewHolder)) {
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (mdragAndSwipeAdapter.isHeadOrFooterView(viewHolder)) {
            return;
        }
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        mDragMoveFlags = mdragAndSwipeAdapter.getDragFlags(recyclerView, viewHolder);
        mSwipeMoveFlags = mdragAndSwipeAdapter.getSwipeFlags(recyclerView, viewHolder);
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
        ...
        return mdragAndSwipeAdapter.getCanMove(recyclerView, source, target);
    }
    ...
    public void onMoved(RecyclerView recyclerView, RecyclerView.ViewHolder source, int fromPos, RecyclerView.ViewHolder target, int toPos, int x, int y) {
        ...
        mdragAndSwipeAdapter.onItemDragMovedCallBack(source, target);
    }

    @Override
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        if (!mdragAndSwipeAdapter.isHeadOrFooterView(viewHolder)) {
            mdragAndSwipeAdapter.onItemSwipedCallBack(viewHolder);
        }
    }
    ...
    public void onChildDrawOver(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE
            ...
            mdragAndSwipeAdapter.onItemSwipingCallBack(c, viewHolder, dX, dY, isCurrentlyActive);
            c.restore();
