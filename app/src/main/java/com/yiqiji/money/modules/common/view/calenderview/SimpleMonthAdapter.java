...
public class SimpleMonthAdapter extends RecyclerView.Adapter<SimpleMonthAdapter.ViewHolder> implements
    ...
    public SimpleMonthAdapter(Context context, DatePickerController datePickerController, TypedArray typedArray,
                              int datePosition, Date reDate, long currentDayPosition, ShowOrHind showOrHind) {
        ...
        calendar2.set(Integer.parseInt(Utils.formatTheDateToMM_dd(reDate, 5)),
                Integer.parseInt(Utils.formatTheDateToMM_dd(reDate, 3)),
                Integer.parseInt(Utils.formatTheDateToMM_dd(reDate, 2)));
        init();
    }
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        v.setId(position);
        // if(showOrHind!=null){
        // if(position>currentDayPosition+1){
        // showOrHind.show(false);
        // }
        // else if(position<currentDayPosition-1){
        // showOrHind.show(true);
        // }
        //
        // }else{
        // showOrHind.hide();
        // }
        if (datePosition != -1 && datePosition == position) {
            v.setRecord_date(day = Integer.parseInt(Utils.formatTheDateToMM_dd(reDate, 2)));
        } else {
            v.setRecord_date(-1);
        }
        ...
        year = position / MONTHS_IN_YEAR + calendar.get(Calendar.YEAR)
                + ((firstMonth + (position % MONTHS_IN_YEAR)) / MONTHS_IN_YEAR) - 30;
        ...
        if (selectedDays.getFirst() != null && selectedDays.getLast() == null) {
            ...
            if (selectedDays.getFirst().month < calendarDay.month) {
                for (int i = 0; i < selectedDays.getFirst().month - calendarDay.month - 1; ++i)
                    mController.onDayOfMonthSelected(selectedDays.getFirst().year, selectedDays.getFirst().month + i,
