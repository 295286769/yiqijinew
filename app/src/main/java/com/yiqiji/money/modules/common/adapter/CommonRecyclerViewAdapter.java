...
/**
 * Created by huangweishui on 2016/4/28. 通用的RecyclerView的适配器 1.支持添加头部
 * 2.支持监听条目点击事件
 * {@link CommonRecyclerViewAdapter#setOnRecyclerViewItemClickListener(OnRecyclerViewItemClickListener)}
 * 3.支持监听条目中的控件的点击事件
 * {@link CommonRecyclerViewAdapter#setOnViewInItemClickListener(OnViewInItemClickListener, int...)}
 */
...
//		}
		if (position < size) {
			...
			if (isNeedSetClickListener(position)) { // 如果需要设置监听
				...
				h.itemView.setOnLongClickListener(new MyLongOnClick(h, position));
			}
			if (viewIdsInItem != null && viewIdsInItem.length > 0 && isNeedSetViewInItemClickListener(position)) {
				for (int i = 0; i < viewIdsInItem.length; i++) {
					...
					if (v != null) {
						...
						v.setOnLongClickListener(new MyLongOnClick(h, viewIdsInItem[i]));
					}
				}
			}
	 ...
	 * 设置条目里面的控件的监听事件
	 * 
	 * @param onViewInItemClickListener
	 *            回掉接口
	 * @param viewIdsInItem
	 *            item中需要监听的view的id数组,可以为null
	 */
	...
	private class MyItemClickListenerAdapter implements View.OnClickListener {
		...
		public void onClick(View v) {
			if (onRecyclerViewItemClickListener != null) {
				// 回调方法
				onRecyclerViewItemClickListener.onItemClick(v, h.getPosition() - headers.size());
			}
		}
	...
	private class MyViewInItemClickListenerAdapter implements View.OnClickListener {
		...
		public void onClick(View v) {
			if (onViewInItemClickListener != null) {
				// 回调方法
				onViewInItemClickListener.onViewInItemClick(v, h.getPosition() - headers.size());
			}
		}

	}

	private class MyLongOnClick implements OnLongClickListener {
		/**
		 * 条目的下标
		 */
		private CommonRecyclerViewHolder h;
		...
		public MyLongOnClick(CommonRecyclerViewHolder h, int viewId) {
			this.h = h;
			this.viewId = viewId;
		}
		...
		public boolean onLongClick(View v) {
			if (onRecyclerViewItemLongClickListener != null) {
				onRecyclerViewItemLongClickListener.onItemLongClick(v, h.getPosition());
			}
			return false;
		}
