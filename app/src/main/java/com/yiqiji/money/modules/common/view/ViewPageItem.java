...
public class ViewPageItem extends ViewGroup {
    ...
    public ViewPageItem(Context context, int line_count) {
        super(context);
        init(context);
    ...
    public ViewPageItem(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int width = getWidth();
        childViewWidth = width / LINE_COUNT;
        int lineWidth = 0;
        int lineHeight = 0;
        // 存储每一行所有的childView
        ...
        for (int i = 0; i < lineNums; i++) {
            ...
            for (int j = 0; j < lineViews.size(); j++) {
                ...
                int rc = lc + childViewWidth;
                int bc = tc + child.getMeasuredHeight();

                child.layout(lc, tc, rc, bc);

                left += childViewWidth + lp.rightMargin + lp.leftMargin;
            }
            left = 0;
            top += lineHeight;
        }
