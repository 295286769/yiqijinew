...
public class AlbumViewPager extends ViewPager implements
		MatrixImageView.OnMovingListener {
	...
	private class ProcressListener implements ImageLoadingProgressListener {
		...
		@Override
		public void onProgressUpdate(String arg0, View view, int current,
				int total) {
			TextView loadText = (TextView) mView
					.findViewById(R.id.current_procress);
	...
	public AlbumViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		localOptions = new DisplayImageOptions.Builder().cacheInMemory(true)
	...
	// ImageLoader.getInstance().displayImage(path.getOriginalUri(),
	// new ImageViewAware(imageView), localOptions,
	// loadingListenerr, new ProcressListener(imageLayout),
	// path.getOrientation());
	//
	// return imageLayout;
	// }
